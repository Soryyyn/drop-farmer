...
module.exports = class Cache {
  ...
  async cacheReleaseList(release, asset) {
    const { url } = asset
    const { token, url: base_url } = this.config
    ...
    for (let i = 0; i < matches.length; i += 1) {
      const asset = release.assets.find(item => item.name === matches[i])
      content = content.replace(matches[i], `https://${base_url}/download/asset/${asset.id}/${asset.name}`)
      console.log(content)
    }
