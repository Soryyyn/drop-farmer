...
export default function Sidebar() {
    ...
    useSendAndWait(api.channels.getFarms, null, (err, response) => {
        if (err) {
            api.log('ERROR', err);
        } else {
            setFarms(response);
     ...
     */
    useHandleOneWay(api.channels.farmStatusChange, farms, (event, response) => {
        /**
         ...
         */
        setFarms(tempCopy);
        api.log('DEBUG', 'Set new farms status');
    });
